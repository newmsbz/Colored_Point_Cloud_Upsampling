...
class dataset_patch(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        input_sparse_patch,input_sparse_normal,input_rgb,gt_dense_patch,gt_dense_normal,gt_rgb=self.basic_set[item,:,0:3],self.basic_set[item,:,3:6], self.basic_set[item,:,6:],self.label_set[item,:,0:3],self.label_set[item,:,3:6],self.label_set[item,:,6:]
        # print(input_sparse_patch[0])
        if self.mode=='train':
            input_sparse_patch, input_sparse_normal, gt_dense_patch, gt_dense_normal,radius=self.augment_data(input_sparse_patch,input_sparse_normal,gt_dense_patch,gt_dense_normal)
        else:
            radius=np.array([1]).astype(np.float32).squeeze()
        # print('check',input_sparse_patch[0])
        return torch.from_numpy(input_sparse_patch.astype(np.float32)).transpose(0,1),torch.from_numpy(input_sparse_normal.astype(np.float32)).transpose(0,1),torch.from_numpy(input_rgb.astype(np.float32)).transpose(0,1),torch.from_numpy(gt_dense_patch.astype(np.float32)).transpose(0,1),torch.from_numpy(gt_dense_normal.astype(np.float32)).transpose(0,1),torch.from_numpy(gt_rgb.astype(np.float32)).transpose(0,1),torch.from_numpy(np.array(radius).astype(np.float32))
