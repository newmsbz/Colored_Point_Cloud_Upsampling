...
def color_mse_loss(gen_rgb, gt_rgb):
    loss = torch.nn.functional.mse_loss(gen_rgb, gt_rgb)
    return loss

def color_mae_loss(gen_rgb, gt_rgb):
    loss = torch.nn.functional.l1_loss(gen_rgb, gt_rgb)
    return loss

def weighted_color_mse_loss(gen_rgb, gt_rgb, weight):
    diff = gt_rgb - gen_rgb
    diff_squared = diff ** 2
    weighted_diff_squared = weight * diff_squared
    mean_diff = weighted_diff_squared.mean()
    return mean_diff